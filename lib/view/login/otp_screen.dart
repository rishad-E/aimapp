import 'dart:developer';
import 'package:aimshala/controllers/login_controller.dart';
import 'package:aimshala/utils/common/colors_common.dart';
import 'package:aimshala/utils/common/text_common.dart';
import 'package:aimshala/utils/common/widgets_common.dart';
import 'package:aimshala/view/login/widget/widgets_login.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:sizer/sizer.dart';

class OTPScreen extends StatefulWidget {
  final String mobileNo;
  const OTPScreen({super.key, required this.mobileNo});

  @override
  State<OTPScreen> createState() => _OTPScreenState();
}

class _OTPScreenState extends State<OTPScreen> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      extendBodyBehindAppBar: true,
      appBar: AppBar(
        automaticallyImplyLeading: false,
        backgroundColor: Colors.transparent,
        surfaceTintColor: Colors.transparent,
        title: appbarContainer(onPressed: () {
          Get.back();
        }),
      ),
      body: Container(
        height: double.infinity,
        decoration: const BoxDecoration(
          image: DecorationImage(
              image: AssetImage('assets/images/bg-image.png'),
              fit: BoxFit.cover),
        ),
        child: Center(
          child: SingleChildScrollView(
            child: GetBuilder(
              init: LoginController(),
              builder: (data) {
                return Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 17),
                  child: Column(
                    children: [
                      Form(
                        key: data.otpFormKey,
                        child: Column(
                          children: [
                            Container(
                              // color: Colors.yellow,
                              padding:
                                  const EdgeInsets.symmetric(horizontal: 5),
                              height: 20.h,
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.end,
                                children: [
                                  SizedBox(
                                    width: 61.w,
                                    height: 7.h,
                                    child: Image.asset(
                                        'assets/images/aimshala-logo.png'),
                                  ),
                                  const SizedBox(
                                    height: 12,
                                  ),
                                  primarytxt2(
                                      'We envision a world where individuals are equipped to Take charge of their lives, realise their aspirations, and make meaningful contributions to society, fostering a future of limitless possibilities.',
                                      9.sp),
                                  const SizedBox(
                                    height: 12,
                                  )
                                ],
                              ),
                            ),
                            SizedBox(
                              height: 42.h,
                              width: double.infinity,
                              child: loginContainer(
                                  twoText: Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: [
                                      twoStyleTexts(
                                          text1: 'Verification Code',
                                          text2: ''),
                                      Container(
                                        padding:
                                            const EdgeInsets.only(bottom: 5),
                                        child: Image.asset(
                                          'assets/images/msg.png',
                                          height: 25,
                                          width: 35,
                                        ),
                                      )
                                    ],
                                  ),
                                  fieldText: primarytxt(
                                      "Please enter the Verification Code send to +91${widget.mobileNo}",
                                      10.7.sp),
                                  context: context,
                                  textfield: SizedBox(
                                    width: 50.8.w,
                                    child: Column(
                                      children: [
                                        Row(
                                          mainAxisAlignment:
                                              MainAxisAlignment.spaceEvenly,
                                          children: [
                                            OTPTextField(
                                              first: true,
                                              last: false,
                                              controller: data.otpController1,
                                              onChanged: (value) {
                                                if (value.length == 1) {
                                                  FocusScope.of(context)
                                                      .nextFocus();
                                                }
                                                onchangeButton(data);
                                              },
                                            ),
                                            OTPTextField(
                                              first: false,
                                              last: false,
                                              controller: data.otpController2,
                                              onChanged: (value) {
                                                if (value.length == 1) {
                                                  FocusScope.of(context)
                                                      .nextFocus();
                                                }
                                                if (value.isEmpty) {
                                                  FocusScope.of(context)
                                                      .previousFocus();
                                                }
                                                onchangeButton(data);
                                              },
                                            ),
                                            OTPTextField(
                                              first: false,
                                              last: false,
                                              controller: data.otpController3,
                                              onChanged: (value) {
                                                if (value.length == 1) {
                                                  FocusScope.of(context)
                                                      .nextFocus();
                                                }

                                                if (value.isEmpty) {
                                                  FocusScope.of(context)
                                                      .previousFocus();
                                                }
                                                onchangeButton(data);
                                              },
                                            ),
                                            OTPTextField(
                                              first: false,
                                              last: true,
                                              controller: data.otpController4,
                                              onChanged: (value) {
                                                if (value.length == 1) {
                                                  FocusScope.of(context)
                                                      .unfocus();
                                                }
                                                if (value.isEmpty) {
                                                  FocusScope.of(context)
                                                      .previousFocus();
                                                }
                                                onchangeButton(data);
                                              },
                                            ),
                                          ],
                                        ),
                                        Obx(() => Text(
                                            data.validationMessage.value,
                                            style: errorStyle())),
                                      ],
                                    ),
                                  ),
                                  otpWidget: SizedBox(
                                    // color: Colors.yellow,
                                    height: 8.h,
                                    child: Column(
                                      mainAxisAlignment:
                                          MainAxisAlignment.spaceAround,
                                      children: [
                                        primarytxt(
                                            "Didn't recieve Code?", 11.sp),
                                        GestureDetector(
                                          onTap: () {
                                            data.resendOTPFunction(
                                                mobileNo: widget.mobileNo);
                                          },
                                          child: Text(
                                            "Resend Code",
                                            style: TextStyle(
                                                color: mainPurple,
                                                fontSize: 13.sp,
                                                fontWeight: FontWeight.w600),
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                  style: ButtonStyle(
                                    backgroundColor: MaterialStateProperty
                                        .resolveWith<Color>(
                                      (Set<MaterialState> states) {
                                        return data.buttonColor.value;
                                      },
                                    ),
                                    shape: MaterialStateProperty.all<
                                            RoundedRectangleBorder>(
                                        RoundedRectangleBorder(
                                            borderRadius:
                                                BorderRadius.circular(10))),
                                  ),
                                  valText: Text(
                                    'Verify',
                                    style: TextStyle(
                                        color: data.buttonTextColor.value),
                                  ),
                                  onPressed: () {
                                    if (data.otpFormKey.currentState!
                                        .validate()) {
                                      String validationMessage =
                                          data.validateOtp();
                                      if (validationMessage.isEmpty) {
                                        String otp = data.otpController1.text +
                                            data.otpController2.text +
                                            data.otpController3.text +
                                            data.otpController4.text;
                                        data.verifyOTPFunction(
                                            mobileNo: widget.mobileNo,
                                            otp: otp);
                                      } else {
                                        log(validationMessage);
                                      }
                                    }
                                  }),
                            ),
                          ],
                        ),
                      ),
                    ],
                  ),
                );
              },
            ),
          ),
        ),
      ),
    );
  }

  void onchangeButton(LoginController data) {
    return setState(() {
      data.updateButtonColor();
    });
  }
}
